...
export async function GET() {
  ...
  try {
    ...
    if (error) {
      return NextResponse.json({ error: "Unable to load attendance data." }, { status: 503 });
    }
    ...
    return NextResponse.json({ error: "Unable to connect to attendance service." }, { status: 503 });
  }
}
